...
const STATIC_REDIRECTS = `# Static page renames
...
# popup.php with no query — fallback to folio
/popup.php                  /folio                301

# Per-work popup.php redirects (image_root → slug)
`;
...
async function main() {
  ...
  // Syntax: /from?image=ROOT  /to  301!
  // The `!` forces the redirect even if the destination doesn't exist (it does, but defensive).
